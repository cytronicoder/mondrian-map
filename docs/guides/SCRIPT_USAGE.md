...
```bash
# Ensure the script has execute permissions (default setting)
...
```
Terminating any active Streamlit processes...
Port 8501 is unavailable, attempting alternate port...
Port 8502 is unavailable, attempting alternate port...
Launching Streamlit on port 8503
...
```bat
REM Execute from the project root directory
...
Terminating any active Streamlit processes...
Port 8501 is unavailable, attempting alternate port...
Launching Streamlit on port 8502
...
If Streamlit is not installed, the following message appears:

```
[ERROR] Streamlit is not installed. Execute: pip install -r config/requirements.txt
```
...
The urllib3 warning about LibreSSL is expected on macOS and does not affect functionality:
