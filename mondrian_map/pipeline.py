...
class MondrianMapPipeline:
    ...
    def _compute_degs(self) -> Tuple[pd.DataFrame, Dict[str, set]]:
        ...
        if self.config.use_cache and deg_cache_path.exists():
            ...
            # Return empty FC df since we don't need it with cached data
            return pd.DataFrame(), deg_sets

        # TODO: Implement full DEG computation from raw expression
        raise NotImplementedError(
            "Full DEG computation requires raw expression data. "
            "Use --use-cache to run from cached artifacts."
        )
