...
# ------------------------------------------------------------
# Square Plot Rendering Helpers
# ------------------------------------------------------------
def _apply_square_figure_layout(fig, size_px: int) -> None:
    """Force a square Plotly figure and 1:1 axis scale.

    Notes:
    - In Streamlit, `width='stretch'` makes Plotly stretch horizontally,
      which can distort a square map into a rectangle.
    - We instead render with an explicit pixel size and center the chart.
    """
    try:
        fig.update_layout(
            width=size_px,
        ...
        # Enforce 1:1 scaling in data coordinates.
        fig.update_yaxes(scaleanchor="x", scaleratio=1)
        fig.update_xaxes(constrain="domain")
    except Exception:
        # If a non-Plotly object is passed, fail silently.
        return


def render_square_plotly(
    fig,
    *,
    key: str,
    size_px: int,
    ...
    """Render a Plotly chart as a centered square and return click selection."""
    _apply_square_figure_layout(fig, size_px=size_px)
    st.markdown(
        f"""<div style="max-width:{size_px}px; margin: 0 auto;">""",
...
def create_detailed_popup(df: pd.DataFrame, dataset_name: str):
    ...
    # ------------------------------
    # Row 1: dedicate the full row to the square figure
    # ------------------------------
    ...
    # Handle pathway clicks for tooltip (same convention as other views)
    if (
        clicked_data
    ...
    # ------------------------------
    # Row 2: dataset statistics (separate row, not beside the figure)
    # ------------------------------
    ...
    # Lower row: color distribution + top pathways
...
def main():
    ...
    if len(df_list) > 0:
        ...
        with canvas_container:
            # If the overview is effectively a single map (1x1), enforce square rendering.
