...
from mondrian_map.degs import (call_degs_by_fc_thresholds, compute_fold_change,
...
def test_call_degs_thresholds_matches_fc_ratio_spec():
    """Test DEG calling with standard thresholds (1.5/0.5).

    Note: These DEG calling thresholds differ from visualization thresholds
    (1.25/0.75 in core.py) which are used for tile coloring in Mondrian maps.
    """
...
def test_compute_profile_degs_returns_two_contrasts_with_expected_counts_on_synthetic_data():
    ...
    degs = compute_profile_degs(
        profile_expr, min_value=0.001, up_threshold=1.5, down_threshold=0.5
    )
...
def test_filter_genes_by_expression_threshold_drops_low_genes_and_records_stats():
    ...
    ftp, fr1, fr2, stats = filter_genes_by_expression_threshold(
        tp, r1, r2, threshold=0.001
    )
